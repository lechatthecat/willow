...
//!     nothing and the whole method was left to the AST emitter. Eligibility
...
//!     all — which was a silent fallback then and is a hard compile error now.
//!   * THE VTABLE SLOT — the same `has_self` mistake in
//!     `register_class_own_vmethods` denied an implicit-self `open`/`override`
//!     method its virtual slot. That one was not an eligibility problem at all:
//!     virtual dispatch found two implementations with nowhere to dispatch
//!     through and the compiler panicked.
//!
//! Since willow-0g8j.3 a body the walker cannot take is a compile error, so a
//! run that prints the right answer is proof the walker produced it. Where a
//! perspective is about WHICH body was walked, the test reads
//! `WILLOW_LIR_LOG=1` rather than the program's output.
//!
//! Perspectives:
...
//!  28. every body in `example/lir_self_statics.wi` compiles and runs
...
// 27. The other half of the receiver defect: an implicit-self method had NO
// lowered IR at all, so the log is what shows it now has one.
#[test]
...
// 28. The shipped example: the same story at program scale.
#[test]
...
// files in `example/` the walker could not compile at all.
#[test]
fn lir_self_29_the_last_two_corpus_holdouts_compile() {
    for (path, expected) in [
        ...
        assert!(ok, "{path} must compile and run:\n{out}");
