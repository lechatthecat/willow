...
pub(crate) fn fold_blocks(blocks: &mut [SourceBlock]) {
    ...
    for block in blocks {
        ...
        for inst in &mut block.instrs {
            ...
            match inst {
                SourceInst::Let { name, value, .. } | SourceInst::Assign { name, value, .. } => {
            ...
            match inst {
                ...
                SourceInst::Defer { body, .. } => regions.push(body.function.blocks.as_mut_slice()),
...
pub(crate) fn eliminate_dead_values(blocks: &mut [SourceBlock], locals: &[super::lowered::LirLocal]) {
    ...
    let synthetic: HashMap<_, _> = locals.iter().filter(|local| local.synthetic).map(|local| (local.name.as_str(), &local.ty)).collect();
    ...
    let pure = |value: &LirRvalue| match value {
        ...
        LirRvalue::Binary { operand_ty: Type::String, .. } => false,
        LirRvalue::Binary { op, rhs, operand_ty, .. } => match op {
    ...
    for block in blocks.iter() {
        for inst in &block.instrs {
            ...
            for &local in &defs { *definitions.entry(local).or_default() += 1; }
            if let SourceInst::Compute { local, value, .. } = inst
                && locals[local.0 as usize].synthetic && pure(value)
            {
                dependencies.insert(*local, value.operands().into_iter().filter_map(|operand|
                    match operand { LirOperand::Local(id) => Some(*id), _ => None }
                ).collect::<Vec<_>>());
            }
        }
    }
    ...
    for block in blocks.iter_mut() {
        ...
        for inst in &block.instrs {
            if matches!(inst, SourceInst::Compute { local, .. } if dependencies.contains_key(local)) { continue; }
            defs.clear();
    ...
    while let Some(local) = pending.pop() {
        if let Some(inputs) = dependencies.get(&local) {
            for &input in inputs {
                if live.insert(input) { pending.push(input); }
            }
        }
    }
    for block in blocks {
...
pub(crate) fn simplify_cfg(blocks: &mut [SourceBlock]) {
    for block in blocks {
        if let SourceTerminator::Branch { cond, then_block, else_block } = &block.terminator
            && let HirExprKind::Bool(value) = cond.kind
        {
            block.terminator = SourceTerminator::Jump(if value { *then_block } else { *else_block });
        }
    }
}
...
pub(crate) fn inline_scalar_leaves(functions: &mut [super::lowered::SourceFunction]) {
    ...
    fn candidate(function: &super::lowered::SourceFunction) -> Option<Leaf> {
        if function.is_async || function.blocks.len() != 1 || !function.captures.is_empty()
            || function.params.iter().any(|param| param.by_reference)
            || !matches!(function.return_type, Type::I64 | Type::F64 | Type::Bool)
            || function.locals.iter().any(|local| !matches!(local.ty, Type::I64 | Type::F64 | Type::Bool))
        { return None; }
        let block = &function.blocks[0];
        if !block.recovery.is_empty() || block.instrs.len() > 8 { return None; }
            ...
            match instruction {
                SourceInst::Compute { local, value, span } => {
                    match value {
                        ...
                        LirRvalue::Binary { op, .. } if !matches!(op, BinOp::Div | BinOp::Rem | BinOp::Pow) => {}
                        _ => return None,
                    }
                    operations.push((*local, value.clone(), function.locals[local.0 as usize].ty.clone(), *span));
                }
                SourceInst::ClearScopeRoots { .. } => {} // all locals proved scalar
                _ => return None,
            }
        }
        let SourceTerminator::Return(Some(result)) = &block.terminator else { return None; };
        Some(Leaf {
            params: function.params.iter().map(|param| names.get(param.name.as_str()).copied()).collect::<Option<_>>()?,
            operations, result: operand(result, &names)?,
        })
    }
    fn remap(operand: &LirOperand, locals: &HashMap<LirLocalId, LirOperand>) -> LirOperand {
        match operand { LirOperand::Local(id) => locals[id].clone(), _ => operand.clone() }
    }
    let leaves: HashMap<FunctionId, Leaf> = functions.iter().filter_map(|function|
        candidate(function).map(|leaf| (function.name, leaf))
    ).collect();
    for function in functions {
        let mut names: std::collections::HashSet<_> = function.locals.iter().map(|local| local.name.clone()).collect();
        for block in &mut function.blocks {
            let mut instructions = Vec::new();
            for instruction in std::mem::take(&mut block.instrs) {
                if let SourceInst::Compute { local, value: LirRvalue::DirectCall { callee, args, .. }, span } = &instruction
                    && let Some(leaf) = leaves.get(callee)
                    && leaf.params.len() == args.len()
                {
                    let mut mapping: HashMap<_, _> = leaf.params.iter().copied().zip(args.iter().cloned()).collect();
                    for (source, value, ty, source_span) in &leaf.operations {
                        let value = match value {
                            LirRvalue::Use(input) => LirRvalue::Use(remap(input, &mapping)),
                            LirRvalue::Unary { op, operand, ty } => LirRvalue::Unary { op: op.clone(), operand: remap(operand, &mapping), ty: ty.clone() },
                            LirRvalue::Binary { op, lhs, rhs, operand_ty } => LirRvalue::Binary { op: op.clone(), lhs: remap(lhs, &mapping), rhs: remap(rhs, &mapping), operand_ty: operand_ty.clone() },
                        ...
                        while !names.insert(name.clone()) { name.push('_'); }
                        function.locals.push(super::lowered::LirLocal { storage_kind: super::lowered::LirStorageKind::Value, id, name, ty: ty.clone(), source_span: None, synthetic: true, parameter: false });
                        instructions.push(SourceInst::Compute { local: id, value, span: *source_span });
                        mapping.insert(*source, LirOperand::Local(id));
                    }
                    instructions.push(SourceInst::Compute { local: *local, value: LirRvalue::Use(remap(&leaf.result, &mapping)), span: *span });
                } else {
        ...
        if function.is_async {
            function.async_frame = super::lowered::async_liveness::analyze(&function.blocks, &function.locals);
        }
    }
}
...
mod tests {
    ...
    fn scalar_object_fixture() -> (Vec<SourceBlock>, Vec<super::super::lowered::LirLocal>) {
        use super::super::lowered::{BlockId, LirLocal, LirLocalId as Id, LirOperand as O, LirRvalue as V};
        let class = crate::semantic::ids::TypeId::local("ScalarPair");
        let object_ty = Type::Named(class);
        let locals = [object_ty.clone(), object_ty.clone(), Type::Void, Type::I64, Type::I64].into_iter().enumerate().map(|(id, ty)| LirLocal { storage_kind: super::super::lowered::LirStorageKind::Value, id: Id(id as u32), name: format!("v{id}"), ty, source_span: None, synthetic: true, parameter: false }).collect();
        let compute = |id, value| SourceInst::Compute { local: Id(id), value, span: Span::dummy() };
        ...
        block.instrs = vec![
            ...
            compute(2, V::FieldStore { object: O::Local(Id(1)), object_ty: object_ty.clone(), field: "x".into(), value: O::Int(17) }),
            compute(3, V::FieldLoad { object: O::Local(Id(1)), object_ty, field: "x".into(), result: Type::I64 }),
            compute(4, V::Binary { op: BinOp::Add, lhs: O::Local(Id(3)), rhs: O::Int(25), operand_ty: Type::I64 }),
        ];
    ...
    fn scalar_replacement_eliminates_aliases_and_preserves_computed_value() {
        ...
        assert!(matches!(&blocks[0].instrs[0], SourceInst::Compute { value: V::Use(super::super::lowered::LirOperand::Int(17)), .. }));
        ...
        for inst in &blocks[0].instrs {
            let SourceInst::Compute { local, value, .. } = inst else { panic!("expected scalar computation") };
            ...
            let result = match value {
                ...
                V::Binary { op: BinOp::Add, lhs, rhs, .. } => read(lhs) + read(rhs),
    ...
    fn scalar_replacement_runs_after_resolved_constructor_lowering() {
        use super::super::lowered::LirRvalue as V;
        let program = lowered("class Pair { pub x: i64; pub y: i64; } fn sum(x: i64) -> i64 { let p = new Pair(x, 25); return p.x + p.y; }");
        let function = program.functions.iter().find(|function| function.name == "sum".into()).unwrap();
        assert!(!function.blocks.iter().flat_map(|block| &block.instrs).any(|inst| matches!(inst, SourceInst::Compute { value: V::ObjectAlloc { .. } | V::FieldStore { .. } | V::FieldLoad { .. }, .. })));
    }

    #[test]
    fn scalar_replacement_tracks_named_aliases() {
        let (mut blocks, locals) = scalar_object_fixture();
        blocks[0].instrs[1] = SourceInst::Let { local: locals[1].id, name: "v1".into(), mutable: false, span: Span::dummy(), ty: locals[1].ty.clone(), value: HirExpr { kind: HirExprKind::Var("v0".into()), ty: locals[0].ty.clone(), span: Span::dummy() } };
        scalar_replace_objects(&mut blocks, &locals);
        assert_eq!(blocks[0].instrs.len(), 2);
    }
    ...
    fn scalar_replacement_keeps_escaping_uninitialized_and_observed_objects() {
        ...
        for scenario in 0..3 {
            let (mut blocks, locals) = scalar_object_fixture();
            match scenario {
                0 => blocks[0].terminator = SourceTerminator::Return(Some(HirExpr { kind: HirExprKind::Var("v1".into()), ty: locals[1].ty.clone(), span: Span::dummy() })),
                1 => { blocks[0].instrs.remove(2); },
                _ => blocks[0].instrs.insert(3, SourceInst::Compute { local: Id(2), value: V::BuiltinCall { callee: crate::semantic::ids::FunctionId::free("gc_allocated_bytes"), args: vec![], params: vec![], result: Type::I64 }, span: Span::dummy() }),
            }
    ...
    fn scalar_replacement_does_not_re_read_a_mutated_stored_operand() {
        ...
        let (mut blocks, locals) = scalar_object_fixture();
        if let SourceInst::Compute { value: V::FieldStore { value, .. }, .. } = &mut blocks[0].instrs[2] { *value = O::Local(Id(4)); }
        blocks[0].instrs.insert(2, SourceInst::Compute { local: Id(4), value: V::Use(O::Int(17)), span: Span::dummy() });
        let before = blocks.clone();
        scalar_replace_objects(&mut blocks, &locals);
    ...
    fn inlines_small_scalar_leaves_but_preserves_faulting_calls() {
        ...
        let program = lowered("fn twice(x: i64) -> i64 { return x * 2; } fn divide(x: i64, y: i64) -> i64 { return x / y; } fn f(x: i64, y: i64) -> i64 { return twice(x) + divide(x, y); }");
        let function = program.functions.iter().find(|f| f.name == "f".into()).unwrap();
        let callees: Vec<_> = function.blocks.iter().flat_map(|block| &block.instrs).filter_map(|inst|
            if let SourceInst::Compute { value: LirRvalue::DirectCall { callee, .. }, .. } = inst { Some(callee.to_string()) } else { None }
        ).collect();
    ...
    fn dce_removes_dead_scalar_dependency_chains() {
        ...
        assert!(!program.functions[0].blocks.iter().flat_map(|block| &block.instrs)
    ...
    fn dce_preserves_faults_and_their_live_inputs() {
        ...
        let operations: Vec<_> = program.functions[0].blocks.iter().flat_map(|block| &block.instrs)
    ...
    fn cfg_removes_only_the_unchosen_constant_arm() {
        let program = lowered("fn effect() {} fn f() { if 1 + 2 == 3 { effect(); } else { effect(); effect(); } }");
        let function = program.functions.iter().find(|f| f.name == "f".into()).unwrap();
        let calls = function.blocks.iter().flat_map(|block| &block.instrs).filter(|inst|
            matches!(inst, SourceInst::Expr(HirExpr { kind: HirExprKind::Call { .. }, .. })
                | SourceInst::Compute { value: super::super::lowered::LirRvalue::DirectCall { .. }, .. })
        ).count();
        assert_eq!(calls, 1);
    }
    ...
    fn lowered_program_exposes_folded_exponent() {
        ...
        assert!(lir.functions[0].blocks.iter().flat_map(|block| &block.instrs).any(|inst|
            matches!(inst, SourceInst::Compute { value: super::super::lowered::LirRvalue::Binary {
                op: BinOp::Pow, rhs: super::super::lowered::LirOperand::Int(3), ..
            }, .. })
        ));
    }
    ...
    fn calls_and_reference_arguments_invalidate_facts() {
        for args in [
            ...
            assert_eq!(blocks[0].terminator, SourceTerminator::Return(Some(var("x"))));
        }
    }
...
pub(crate) fn scalar_replace_objects(blocks: &mut [SourceBlock], locals: &[super::lowered::LirLocal]) {
    use super::lowered::{LirLocalId, LirOperand as O, LirRvalue as V};
    use super::lowered::async_liveness::{instruction_use_def, terminator_uses};
    use std::collections::HashSet;
    let [block] = blocks else { return; };
    if !block.recovery.is_empty() { return; }
    let scalar = |ty: &Type| matches!(ty, Type::I64 | Type::F64 | Type::Bool);
    let atom = |expr: &HirExpr| scalar(&expr.ty) && matches!(expr.kind, HirExprKind::Int(_) | HirExprKind::Float(_) | HirExprKind::Bool(_) | HirExprKind::Var(_));
    if !matches!(&block.terminator, SourceTerminator::Return(None)) && !matches!(&block.terminator, SourceTerminator::Return(Some(value)) if atom(value)) { return; }
    let names: HashMap<_, _> = locals.iter().map(|local| (local.name.as_str(), local.id)).collect();
    ...
    for (index, inst) in block.instrs.iter().enumerate() {
        ...
        for &id in &defs { last_def.insert(id, index); *counts.entry(id).or_default() += 1; }
        if let SourceInst::Compute { local, value: V::ObjectAlloc { class }, .. } = inst {
            if candidate.replace((*local, *class, index)).is_some() { return; }
        }
    }
    let Some((object, class, allocated_at)) = candidate else { return; };
    if counts.get(&object) != Some(&1) { return; }
    ...
    for (index, inst) in block.instrs.iter().enumerate() {
        if index == allocated_at { continue; }
        let alias_source = match inst {
            SourceInst::Compute { local, value: V::Use(O::Local(source)), .. } if aliases.contains(source) => Some((*local, *source)),
            SourceInst::Let { name, value: HirExpr { kind: HirExprKind::Var(source), .. }, .. } => names.get(source.as_str()).filter(|source| aliases.contains(source)).and_then(|source| names.get(name.as_str()).map(|target| (*target, *source))),
            _ => None,
        };
        if let Some((target, _)) = alias_source {
            if index < allocated_at { return; }
            if counts.get(&target) != Some(&1) { return; }
            aliases.insert(target); remove.insert(index); continue;
        }
        match inst {
            SourceInst::Compute { local, value: V::FieldStore { object: O::Local(owner), object_ty, field, value }, .. } if aliases.contains(owner) => {
                if !matches!(object_ty, Type::Named(name) if *name == class) || !value.ty(locals).is_some_and(|ty| scalar(&ty)) { return; }
                if let O::Local(source) = value {
                    if aliases.contains(source) || last_def.get(source).is_some_and(|defined| *defined > index) { return; }
                }
                fields.insert(field.clone(), value.clone());
                discard_defs.insert(*local); remove.insert(index); continue;
            }
            SourceInst::Compute { value: V::FieldLoad { object: O::Local(owner), object_ty, field, result }, .. } if aliases.contains(owner) => {
                let Some(value) = fields.get(field) else { return; };
                if !matches!(object_ty, Type::Named(name) if *name == class) || !scalar(result) || value.ty(locals).as_ref() != Some(result) { return; }
                replacements.insert(index, value.clone()); continue;
            }
            SourceInst::Compute { value, .. } => match value {
                V::Use(operand) if operand.ty(locals).is_some_and(|ty| scalar(&ty)) => {},
                V::Unary { ty, .. } if scalar(ty) => {},
                V::Binary { op, operand_ty, .. } if scalar(operand_ty) && !matches!(op, BinOp::Div | BinOp::Rem | BinOp::Pow) => {},
                _ => return,
            },
            SourceInst::Let { value, .. } | SourceInst::Assign { value, .. } | SourceInst::Expr(value) if atom(value) => {},
            SourceInst::ClearScopeRoots { .. } => continue,
            _ => return,
        }
        uses.clear(); defs.clear();
        instruction_use_def(inst, &names, &mut uses, &mut defs);
        if !uses.is_disjoint(&aliases) || !defs.is_disjoint(&aliases) { return; }
    }
    ...
    for (index, inst) in block.instrs.iter().enumerate() {
        if remove.contains(&index) || matches!(inst, SourceInst::ClearScopeRoots { .. }) { continue; }
        uses.clear(); defs.clear();
        instruction_use_def(inst, &names, &mut uses, &mut defs);
        if !uses.is_disjoint(&discard_defs) { return; }
    }
    uses.clear();
    terminator_uses(&block.terminator, &names, &mut uses, &HashSet::new());
    if !uses.is_disjoint(&aliases) || !uses.is_disjoint(&discard_defs) { return; }
    let mut index = 0;
    block.instrs.retain_mut(|inst| {
        let at = index; index += 1;
        if remove.contains(&at) { return false; }
        if let Some(operand) = replacements.remove(&at) {
            if let SourceInst::Compute { value, .. } = inst { *value = V::Use(operand); }
        }
        true
    });
}
