...
fn static_field(class: &TypeId, field: &str, callables: &Callables) -> bool {
    ...
    while let Some(class) = next {
        ...
        let Some(info) = callables.resolution.classes.get(&class) else {
            return false;
        };
        if info.static_fields.contains_key(field) {
            return true;
        }
        next = info.base;
    }
...
fn static_call(
    ...
    if let Some(signature) = super::super::static_signature(&callables.resolution, class, method) {
        return signature.is_static
            && (if signature.is_async {
                Type::Generic(TypeId::local("Task"), vec![signature.return_type.clone()])
            } else {
    ...
    if callables.resolution.classes.contains_key(class)
        || callables.resolution.enums.contains_key(class)
    {
        return false;
    }
    ...
    match (class_name.as_str(), method, args) {
        (
            "Map" | "Channel" | "CancelToken" | "CancelSource" | "CancellationToken" | "TaskScope",
            "new",
            [],
        ) => true,
        ("Channel", "with_capacity", [arg]) => arg.ty == Type::I64,
        (
            "AtomicI64" | "AtomicBool" | "CellI64" | "CellBool" | "Mutex" | "RwLock"
            | "BlockingCell" | "BlockingRwCell",
            "new",
            [arg],
        ) => {
            use crate::semantic::builtin_types;
            matches!(result, Type::Generic(name, payloads) if name == class && matches!(payloads.as_slice(), [ty] if argument_coercible(ty, arg, &callables.resolution)))
                || builtin_types::resolve(result).is_some_and(|resolved| {
                    resolved
                        .args
                        .first()
                        .is_some_and(|ty| argument_coercible(ty, arg, &callables.resolution))
                })
                || matches!(
                    (class_name.as_str(), &arg.ty),
                    ("AtomicI64" | "CellI64", Type::I64) | ("AtomicBool" | "CellBool", Type::Bool)
                )
        }
        _ => false,
    }
...
fn enum_payloads(
    ...
    let info = callables.resolution.enums.get(class)?;
    let args = match ty {
        Type::Generic(name, args) if name == class => args.as_slice(),
        Type::Named(name) if name == class => &[],
        _ => return None,
    };
    ...
    let mut payloads: Vec<_> = variant
        .payloads
        .iter()
        .map(|payload| crate::semantic::symbols::substitute_type(payload, &substitutions))
        .collect();
    if payloads == [Type::Void] {
        payloads.clear();
    }
    Some(payloads)
}
...
fn constructor_supported(class: &TypeId, args: &[HirExpr], callables: &Callables) -> bool {
    ...
    if let Some(signature) = &info.constructor {
        ...
        class_fields(class, callables).is_some_and(|fields| fields.len() == args.len())
    }
}
...
fn builtin_call(callee: &FunctionId, args: &[HirExpr], result: &Type, is_async: bool) -> bool {
    ...
    if name == "panic" {
        return *result == Type::Never
            && match args {
                [] => true,
                [arg] => arg.ty == Type::String,
                _ => format_segments(args).is_some(),
            };
    }
    if name == "format" {
        return *result == Type::String && format_segments(args).is_some();
    }
    if name == "recover" {
    ...
    crate::semantic::intrinsics::builtin_call_runtime_name(name).is_some()
        && args.is_empty()
        && if matches!(name, "gc_collect" | "gc_minor_collect") {
            *result == Type::Void
        } else {
            *result == Type::I64
        }
...
fn instance_method(receiver: &Type, method: &str, args: &[HirExpr], callables: &Callables) -> bool {
    super::super::method_signature(&callables.resolution, receiver, method).is_some_and(
        |signature| {
            // Interface dispatch keeps its hidden receiver even for the
            // supported declaration spelling without an explicit `self`.
            (!signature.is_static || matches!(receiver, Type::Named(name) | Type::Generic(name, _) if callables.resolution.interfaces.contains_key(name)))
                && signature.params.len() == args.len()
                && signature
                    .params
                    .iter()
...
fn argument_types(
    node: &HirExpr,
    names: &HashMap<String, LirLocalId>,
    locals: &[LirLocal],
    callables: &Callables,
) -> Option<Vec<Type>> {
    match &node.kind {
        HirExprKind::Call { callee, .. } => {
            if let Some(local) = names.get(callee.unqualified_name()) {
                ...
                callables.get(callee).map(|(params, _)| params.clone())
            }
        }
        HirExprKind::New { class, .. } => callables
            .resolution
            .classes
            .get(class)?
            .constructor
        ...
        HirExprKind::MethodCall { object, method, .. } => {
            super::super::method_signature(&callables.resolution, &object.ty, method)
...
fn lower(
    ...
    let snapshots = expr.walk_postorder(false).any(|node| {
        matches!(
            node.kind,
            HirExprKind::Call { .. }
                | HirExprKind::MethodCall { .. }
    ...
    while let Some(action) = work.pop() {
        let node = match action {
            ...
            Work::AwaitBuiltin(node) => {
                let HirExprKind::Await { inner } = &node.kind else {
                    unreachable!()
                };
                ...
                let future = emit(
                    LirRvalue::BuiltinCall {
                        callee: *callee,
                        args: args
                            .iter()
                            .map(|arg| values[&std::ptr::from_ref(arg)].clone())
                            .collect(),
                        params: args.iter().map(|arg| arg.ty.clone()).collect(),
                        result: future_type.clone(),
                    },
                    future_type,
                    inner.span,
                    emitted,
                ...
                let result = emit(
                    LirRvalue::AwaitFuture {
                        future: LirOperand::Local(future),
                        result: Type::Void,
                    },
                    Type::Void,
                    node.span,
            ...
            Work::Reference(node) => {
                let HirExprKind::ReferenceArg { place } = &node.kind else {
                    unreachable!()
                };
                let LirOperand::Reference {
                    place: captured, ..
                } = values[&std::ptr::from_ref(node)].clone()
                else {
                    unreachable!()
                };
                match (&place.kind, captured) {
                    ...
                    (
                        ...
                        LirPlace::Field { object: local, .. },
                    ) => {
                        emitted.push(SourceInst::Compute {
                            local,
                            value: LirRvalue::Use(values[&std::ptr::from_ref(&**object)].clone()),
                            span: object.span,
                        });
                    }
                    (
                        ...
                        emitted.push(SourceInst::Compute {
                            ...
                            value: LirRvalue::CaptureArrayOwner {
                                array: values[&std::ptr::from_ref(&**array)].clone(),
                                index: LirOperand::Local(local),
                            },
                            span: place.span,
                        });
                    }
            ...
            Work::FieldStore {
                ...
                emit(
                    LirRvalue::FieldStore {
                        ...
                        object_ty,
                        field,
                        value: values[&std::ptr::from_ref(child)].clone(),
                    },
                    Type::Void,
                    child.span,
                    emitted,
            ...
            Work::EnumStore {
                ...
                let LirOperand::Local(local) = object else {
                    unreachable!()
                };
                emitted.push(SourceInst::Compute {
                    local,
            ...
            Work::Raise { node, message } => {
                let message = message
                    .map(|child| values[&std::ptr::from_ref(child)].clone())
                    .unwrap_or_else(|| values[&std::ptr::from_ref(node)].clone());
                let result = emit(
            ...
            Work::Enter(node) => {
                ...
                let reference_call = match &node.kind {
                    ...
                    HirExprKind::New { class, args } => {
                        Some((FunctionId::method(*class, "init"), args))
                    }
                    HirExprKind::StaticCall {
                        class,
                        method,
                        args,
                    } => Some((FunctionId::method(*class, method), args)),
                    _ => None,
                };
                if let Some((callee, args)) = reference_call {
                    ...
                    if has_refs
                        && !matches!(
                            node.kind,
                            HirExprKind::MethodCall { .. } | HirExprKind::New { .. }
                        )
                    {
                        emit(
                            LirRvalue::BeginReferenceCall,
                ...
                match &node.kind {
                    ...
                    HirExprKind::ReferenceArg { place } => {
                        let captured = match &place.kind {
                            ...
                            HirExprKind::FieldAccess { object, field } => LirPlace::Field {
                                object: reserve(object.ty.clone(), locals, names),
                            ...
                            HirExprKind::Index { .. } => {
                                ...
                                locals[owner.0 as usize].storage_kind =
                                    super::super::LirStorageKind::GcOwner;
                                LirPlace::ArrayElement {
                                    owner,
                                    index: reserve(Type::I64, locals, names),
                        ...
                        let argument = LirOperand::Reference {
                            place: captured,
                            span: node.span,
                            display: super::super::reference_place_name(place),
                        };
                        if let Some((callee, index)) =
                            reference_sites.get(&std::ptr::from_ref(node))
                        {
                            emit(
                        ...
                        match &place.kind {
                            ...
                            HirExprKind::Index { array, index } => {
                                work.push(Work::Enter(index));
                                work.push(Work::Enter(array));
                            }
                            _ => unreachable!("validated reference place"),
                        }
                        continue;
                    ...
                    HirExprKind::StaticCall {
                        ...
                        method,
                        args,
                    } if callables.resolution.enums.contains_key(class) => {
                        let object = LirOperand::Local(emit(
                            LirRvalue::EnumAlloc {
                                class: *class,
                    ...
                    HirExprKind::New { class, args } => {
                        let object = LirOperand::Local(emit(
                            LirRvalue::ObjectAlloc { class: *class },
                            node.ty.clone(),
                            node.span,
                            emitted,
                            locals,
                        ...
                        if callables.resolution.classes[class].constructor.is_some() {
                            ...
                            for ((field, _), child) in fields.into_iter().zip(args).rev() {
                                work.push(Work::FieldStore {
                                    object: object.clone(),
                                    object_ty: node.ty.clone(),
                                    field,
                                    child,
                                });
                                work.push(Work::Enter(child));
                ...
                if let HirExprKind::Call { callee, args } = &node.kind {
                    ...
                    if panic {
                        ...
                        if args.is_empty() {
                            work.push(Work::FormatLiteral {
                                node,
                                text: "explicit panic".into(),
                            });
                            continue;
                        }
                        if let Some(message) = message {
                            work.push(Work::Enter(message));
                            continue;
                        }
                    }
                    if (callee.is_free_named("format") || panic) && !callables.contains_key(callee)
                    {
                        ...
                        if segments.is_empty() {
                            actions.push(Work::FormatLiteral {
                                node,
                                text: String::new(),
                            });
                        }
                        for segment in segments {
                            match segment {
                                crate::interpolate::Segment::Literal(text) => {
                                    actions.push(Work::FormatLiteral { node, text })
                                }
                                segment => {
                                    let child = operands.next().expect("validated placeholder");
                                    let format = match segment {
                                        crate::interpolate::Segment::F64(format) => Some(format),
                                        _ => None,
                                    };
                ...
                if let HirExprKind::Array { elements } = &node.kind {
                    ...
                    let array = LirOperand::Local(emit(
                        LirRvalue::ArrayAlloc {
                            length: elements.len(),
                ...
                work.push(Work::Finish(node));
                if let HirExprKind::Call { args, .. } | HirExprKind::StaticCall { args, .. } =
                    &node.kind
                {
                    work.extend(arguments(args, targets.as_deref()));
        ...
        let operand = match &node.kind {
            HirExprKind::Await { inner } => LirOperand::Local(emit(
                LirRvalue::AwaitFuture {
                    future: values[&std::ptr::from_ref(&**inner)].clone(),
            ...
            HirExprKind::Index { array, index } => LirOperand::Local(emit(
                LirRvalue::Index {
                    array: values[&std::ptr::from_ref(&**array)].clone(),
                    index: values[&std::ptr::from_ref(&**index)].clone(),
                    element: node.ty.clone(),
                },
                node.ty.clone(),
                node.span,
                emitted,
            ...
            HirExprKind::FieldAccess { object, field } => LirOperand::Local(emit(
                LirRvalue::FieldLoad {
                    object: values[&std::ptr::from_ref(&**object)].clone(),
                    object_ty: object.ty.clone(),
                    field: field.clone(),
                    result: node.ty.clone(),
                },
            ...
            HirExprKind::StaticField { class, field } => {
                let value = if callables.resolution.enums.contains_key(class) {
                    LirRvalue::EnumAlloc {
                        class: *class,
                        variant: field.clone(),
                        enum_ty: node.ty.clone(),
            ...
            HirExprKind::Range { start, end } => LirOperand::Local(emit(
                LirRvalue::Range {
                    start: values[&std::ptr::from_ref(&**start)].clone(),
                    end: values[&std::ptr::from_ref(&**end)].clone(),
                },
                node.ty.clone(),
                node.span,
                emitted,
            ...
            HirExprKind::Str(value) => LirOperand::Local(emit(
                LirRvalue::StringLiteral(value.clone()),
                node.ty.clone(),
                node.span,
                emitted,
                locals,
            ...
            HirExprKind::Lambda { id, captures, .. } => {
                let value = LirRvalue::Closure {
                    id: *id,
                    captures: captures
                        .iter()
                        .map(|capture| LirOperand::Local(names[&capture.source]))
                        .collect(),
                    ty: node.ty.clone(),
                };
                LirOperand::Local(emit(
            ...
            HirExprKind::Print { value, newline } => {
                let value = LirRvalue::Print {
                    value: values[&std::ptr::from_ref(&**value)].clone(),
                    ty: value.ty.clone(),
                    newline: *newline,
                };
                LirOperand::Local(emit(
            ...
            HirExprKind::MethodCall {
                ...
                let value = if let Some(resolved) =
                    crate::semantic::intrinsics::resolve(&object.ty, method, args.len())
                {
                ...
                } else if enum_method(&object.ty, method) {
            ...
            HirExprKind::Call { callee, args } => {
                ...
                let value = if let Some(callable) = values.get(&std::ptr::from_ref(node)) {
                    ...
                    let (Type::Fn(params, result) | Type::Closure(params, result)) = &ty else {
                        unreachable!()
                    };
                    LirRvalue::IndirectCall {
                        callee: callable.clone(),
                        name: *callee,
                        args: operands,
                        params: params.clone(),
                        result: (**result).clone(),
                    ...
                    if let Some(output) = callables.async_outputs.get(callee) {
                        LirRvalue::StartTask {
                            callee: *callee,
                            args: operands,
                            params: params.clone(),
                ...
                } else if callee.is_free_named("recover") {
                    LirRvalue::Recover
                } else {
                    LirRvalue::BuiltinCall {
                        callee: *callee,
...
fn append_format(
    node: &HirExpr,
    piece: LirOperand,
    values: &mut HashMap<*const HirExpr, LirOperand>,
    emitted: &mut Vec<SourceInst>,
    locals: &mut Vec<LirLocal>,
    names: &mut HashMap<String, LirLocalId>,
) {
...
pub(in crate::ir::lowered) fn lower_blocks(
...
pub(in crate::ir::lowered) fn lower_calls(
    functions: &mut [super::super::SourceFunction],
    lambdas: &mut [super::super::SourceLambda],
    resolution: &crate::ir::typed_ast::HirResolution,
) {
    let mut callables = Callables {
        signatures: HashMap::new(),
        async_outputs: HashMap::new(),
    ...
    for (name, signature) in &resolution.functions {
        ...
        let result = if signature.is_async {
            callables
                .async_outputs
                .insert(*name, signature.return_type.clone());
            Type::Generic(TypeId::local("Task"), vec![signature.return_type.clone()])
        } else {
            signature.return_type.clone()
        };
        callables
            .signatures
            .insert(*name, (signature.params.clone(), result));
    }
    for f in functions.iter() {
        let result = if f.is_async {
            callables
                .async_outputs
                .insert(f.name, f.return_type.clone());
            Type::Generic(TypeId::local("Task"), vec![f.return_type.clone()])
        } else {
            f.return_type.clone()
        };
        callables.signatures.insert(
            f.name,
            (
    ...
    let mut pending: Vec<_> = functions
        .iter_mut()
        ...
        for block in &mut f.blocks {
            for inst in &mut block.instrs {
                if let SourceInst::Defer { body, .. } = inst {
                    pending.push(&mut body.function);
                }
            }
        }
    }
...
fn lower_store(
    ...
    let valid = match &*inst {
        SourceInst::FieldAssign { object, value, .. } => [object, value]
            .iter()
            .all(|expr| scalar(expr, names, locals, callables, is_async)),
        ...
        SourceInst::StaticFieldAssign { value, .. } => {
            scalar(value, names, locals, callables, is_async)
        }
        _ => false,
    };
    if !valid {
    ...
    for block in blocks {
        ...
        for mut inst in std::mem::take(&mut block.instrs) {
            ...
            match &mut inst {
                ...
                SourceInst::Assign { value, .. } | SourceInst::Expr(value) => {
                    lower(value, &mut emitted, locals, &mut names, callables, is_async)
                }
                _ => {}
            }
...
mod tests {
    ...
    fn array_reference_owner_is_captured_before_later_argument_cfg() {
        let program = program(
            "fn write(value: &mut i64, ignored: i64) { value = 9; } fn f(c: bool) { let a = [1]; write(&a[0], c ? gc_allocated_bytes() : 0); }",
        );
        let function = program
            .functions
            .iter()
            .find(|function| function.name.is_free_named("f"))
            .unwrap();
        let capture = function.blocks[0]
            .instrs
            .iter()
            .position(|inst| {
                matches!(
            ...
            .expect("capture before branch");
        let debug = function.blocks[0]
            .instrs
            .iter()
            .position(|inst| {
                matches!(
                    inst,
                    SourceInst::Compute {
                        value: LirRvalue::ReferenceDebug { .. },
                        ..
                    }
                )
            })
    ...
    fn propagation_is_cfg_and_reuses_the_existing_error_object() {
        let program = program(
            r#"fn f(value: Result<i64, String>) -> Result<String, String> { let number = value?; return Result::Ok("ok"); }"#,
        );
        ...
        assert!(
            function
                ...
                .any(|inst| matches!(
                    inst,
                    SourceInst::Compute {
                        value: LirRvalue::RebindResultError { .. },
                        ..
                    }
                ))
        );
        for block in &function.blocks {
            for inst in &block.instrs {
                if let SourceInst::Let { value, .. }
                | SourceInst::Assign { value, .. }
                | SourceInst::Expr(value) = inst
                {
                    assert!(
                        !value
                            .walk_postorder(false)
                            .any(|node| matches!(node.kind, HirExprKind::TryPropagate { .. }))
                    );
                }
            }
        }
    ...
    fn constructor_shell_precedes_conditional_argument_cfg() {
        let program = program(
            "class Box { pub value: i64; } fn f(c: bool) -> Box { return new Box(c ? gc_allocated_bytes() : 0); }",
        );
        let function = &program.functions[0];
        assert!(function.blocks[0].instrs.iter().any(|inst| matches!(
            inst,
            SourceInst::Compute {
                value: LirRvalue::ObjectAlloc { .. },
                ..
            }
        )));
        assert!(matches!(
    ...
    fn enum_shell_precedes_payload_evaluation() {
        let program = program(
            "enum Pair { Values(i64, i64) } fn f() -> Pair { return Pair::Values(gc_allocated_bytes(), gc_allocated_bytes()); }",
        );
        let kinds: Vec<_> = program.functions[0]
            .blocks
            .iter()
            .flat_map(|block| &block.instrs)
    ...
    fn formatting_finishes_each_segment_before_next_operand() {
        let program = program(
            r#"fn f() -> String { return format("{}:{}", gc_allocated_bytes(), gc_allocated_bytes()); }"#,
        );
        let kinds: Vec<_> = program.functions[0]
            .blocks
            .iter()
            ...
            .filter_map(|inst| match inst {
                ...
                SourceInst::Compute {
                    value: LirRvalue::StringLiteral(_),
                    ..
                } => Some("literal"),
                SourceInst::Compute {
                    value:
        ...
        assert_eq!(
            kinds,
            [
                "call", "format", "literal", "concat", "call", "format", "concat"
            ]
        );
    }
    ...
    fn array_allocation_and_stores_interleave_element_evaluation() {
        let program = program(
            "fn f() -> Array<i64> { return [gc_allocated_bytes(), gc_allocated_bytes()]; }",
        );
        let values: Vec<_> = program.functions[0]
            .blocks
            .iter()
            .flat_map(|block| &block.instrs)
            .filter_map(|inst| {
        ...
        let kinds: Vec<_> = values
            ...
            .collect();
        assert_eq!(kinds, ["alloc", "call", "store", "call", "store"]);
        for inst in program.functions[0]
            .blocks
            .iter()
            .flat_map(|block| &block.instrs)
        {
            if let SourceInst::Compute { local, value, .. } = inst {
                assert!(
                    value.is_well_typed(&program.functions[0].locals, *local),
                    "{value:?}"
                );
            }
        }
